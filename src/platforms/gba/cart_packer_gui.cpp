...
namespace {
    ...
    static bool hasEmbeddedTemplate() {
        HRSRC r = FindResourceA(nullptr, MAKEINTRESOURCEA(kTemplateResourceId), RT_RCDATA);
        return r != nullptr;
    }
    ...
    static bool loadEmbeddedTemplate(std::vector<uint8_t>& out, std::string& err) {
        HRSRC r = FindResourceA(nullptr, MAKEINTRESOURCEA(kTemplateResourceId), RT_RCDATA);
        if (!r) {
            err = "Embedded template ROM not found in this executable.";
            return false;
        }
        DWORD sz = SizeofResource(nullptr, r);
        if (sz == 0) {
            err = "Embedded template ROM is empty.";
            return false;
        }
        HGLOBAL h = LoadResource(nullptr, r);
        if (!h) {
            err = "Failed to load embedded template ROM resource.";
            return false;
        }
        void* p = LockResource(h);
        if (!p) {
            err = "Failed to access embedded template ROM resource.";
            return false;
        }
    ...
    static bool loadIniIntValue(const char* section, const char* key, int& outValue) {
        if (!g_iniPath[0] || !section || !key) return false;
        char buf[32] = "";
        GetPrivateProfileStringA(section, key, "", buf, (DWORD)sizeof(buf), g_iniPath);
        if (!buf[0]) return false;
        outValue = std::atoi(buf);
        return true;
    }

    static void saveIniIntValue(const char* section, const char* key, int value) {
        if (!g_iniPath[0] || !section || !key) return;
        char buf[16] = "";
        std::snprintf(buf, sizeof(buf), "%d", value ? 1 : 0);
        WritePrivateProfileStringA(section, key, buf, g_iniPath);
    }

    static void loadToggleStateFromIni(ToggleState& state) {
        int v = 0;
        if (loadIniIntValue("Toggles", "enableAudio", v)) state.enableAudio = v ? 1 : 0;
        if (loadIniIntValue("Toggles", "skipVblank", v)) state.skipVblank = v ? 1 : 0;
        if (loadIniIntValue("Toggles", "profileGba", v)) state.profileGba = v ? 1 : 0;
        if (loadIniIntValue("Toggles", "customSkin", v)) state.cflahBase = v ? 1 : 0;
    }

    static void saveToggleStateToIni(const ToggleState& state) {
        saveIniIntValue("Toggles", "enableAudio", state.enableAudio);
        saveIniIntValue("Toggles", "skipVblank", state.skipVblank);
        saveIniIntValue("Toggles", "profileGba", state.profileGba);
        saveIniIntValue("Toggles", "customSkin", state.cflahBase);
    }
    ...
    static void setSelectedCustomSkin(const char* path) {
        ...
        setCheckboxState(g_toggleCflahBase, 1);
        saveToggleStateToIni(readToggleStateFromUi());
        updateGenerateEnabled();
    }
    ...
    static void handleReset() {
        ...
        saveToggleStateToIni(kDefaultToggles);
        updateGenerateEnabled();
    }
    ...
    LRESULT CALLBACK windowProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
        ...
        case WM_CREATE: {
            ...
            CreateWindowExA(
                0,
                "STATIC",
                "Standalone PICO-8 packer to GBA Rom",
                WS_CHILD | WS_VISIBLE,
                kPadding,
                kPadding,
            ...
            CreateWindowExA(
                0,
                "STATIC",
                "https://github.com/naturelgass/real8-explorer/tree/main/src/platforms/gba",
                WS_CHILD | WS_VISIBLE,
                kPadding,
                kPadding + 18,
                rect.right - (kPadding * 2),
            ...
            const int step4BaseY = kPadding + 44;
            ...
            // Load per-user toggle state (fallback to built-in defaults).
            ToggleState toggles = kDefaultToggles;
            loadToggleStateFromIni(toggles);
        ...
        case WM_COMMAND: {
            ...
            if (id == kIdBrowseCart) {
                ...
                handleGenerate(hwnd);
            } else if (id == kIdToggleAudio || id == kIdToggleCflahBase || id == kIdToggleSkipVblank || id == kIdToggleProfile) {
                saveToggleStateToIni(readToggleStateFromUi());
            }
            break;
        }
